...
use actix_web::{App, cookie::Key, Error, HttpRequest, HttpResponse, HttpServer, middleware, web};
...
async fn main() ->io::Result<()> {
    ...
    let httpServer = HttpServer::new(|| {
        ...
        .workers(2)
        .run();
    let wsServer = HttpServer::new(move || {
        App::new()
        ...
        .bind(("127.0.0.1", 3000))?
        .run();
    future::try_join(httpServer, wsServer).await?;
